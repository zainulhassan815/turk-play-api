...
const parseQueryParams = (params: { [key: string]: string | undefined }): ApiParams => {
  const sortBy: 'ASC' | 'DESC' =
    params.sortBy && (params.sortBy === 'ASC' || params.sortBy === 'DESC') ? params.sortBy : DefaultApiParams.sortBy;

  const sortField: 'id' | 'title' | 'createdAt' =
    params.sortField && (params.sortField === 'id' || params.sortField === 'title' || params.sortField === 'createdAt')
      ? params.sortField
